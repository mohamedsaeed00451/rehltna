...
@section('content')
    <div class="breadcrumb-header justify-content-between mb-4 mt-3">
        <div class="my-auto">
            <h4 class="content-title mb-0 fw-bold">Create Package</h4>
        </div>
    </div>
        ...
        <div class="card-body p-4">
            <form action="{{ route('packages.store') }}" method="POST">
                ...
                <div class="row mb-4">
                    @foreach(get_active_langs() as $lang)
                        <div class="col-md-3 mb-3">
                            <label class="fw-bold mb-2">Name ({{ strtoupper($lang) }}) <span class="text-danger">*</span></label>
                            <input type="text" name="name_{{ $lang }}" class="form-control" placeholder="Name in {{ strtoupper($lang) }}" required>
                        </div>
                    @endforeach
                    ...
                    <div class="col-md-2 mb-3">
                        <label class="fw-bold mb-2">Points Multiplier <i class="fas fa-info-circle text-primary" title="x1 for standard, x2 to double points"></i></label>
                        <div class="input-group">
                            <span class="input-group-text bg-light fw-bold">X</span>
                            <input type="number" step="0.1" name="points_multiplier" class="form-control" placeholder="e.g. 2" value="1" required>
                        </div>
                    </div>
                    ...
                    <div class="col-md-2 mb-3">
                        ...
                        <select name="icon" class="form-select fw-bold" required>
                            <option value="fas fa-cube">📦 Box</option>
                            ...
                            <div class="row">
                                @foreach(get_active_langs() as $lang)
                                    <div class="col-md mt-2 mt-md-0">
                                        <input type="text" name="feature_{{ $lang }}[]" class="form-control bg-white"
                ...
                <div class="mt-4 pt-3 text-end">
                    ...
                    <button type="submit" class="btn btn-primary rounded-pill px-5">Save Package</button>
                </div>
            </form>
        </div>
    </div>
...
@section('scripts')
    <script>
        // Generate Template from Blade explicitly for JS
