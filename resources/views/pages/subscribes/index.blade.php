...
@section('content')
    ...
    <div class="breadcrumb-header justify-content-between mb-4 mt-3">
        <div class="my-auto">
            ...
            <p class="text-muted mb-0 small fw-medium">Marketing / <span class="text-primary">Newsletter List</span></p>
        </div>
    </div>

    <div class="hero-section">
        <div>
            <h3 class="mb-2 fw-bold" style="letter-spacing: -0.5px;">Subscription Hub</h3>
            <p class="mb-0 opacity-75 fw-medium">Manage your newsletter audience and export subscriber data for
                campaigns.</p>
        </div>
        <div class="d-flex gap-3 align-items-center">
            ...
            <a href="{{ route('subscribes.export-excel') }}"
                    ...
                    <div class="flex-grow-1" style="max-width: 400px;">
                        <div class="input-group">
                            ...
                            <input autofocus type="text" id="search" class="form-control-deluxe border-start-0"
                                   style="border-radius: 0 12px 12px 0 !important;" placeholder="Search by email..."
                                   value="{{ request('search') }}">
                        </div>
                    </div>
                </div>

                <form method="POST" action="{{ route('subscribes.bulk-delete') }}" id="bulk-delete-form">
                    @csrf
                    <div class="card-body p-0" id="subscribes-table">
                        @include('pages.subscribes.partials.table', ['subscribes' => $subscribes])
                    </div>
                </form>

                <div class="card-footer bg-transparent border-0 p-4 mt-2">
                    <div class="d-flex justify-content-between align-items-center">
                        <p class="text-muted small mb-0 fw-bold">Total Subscribers: {{ $subscribes->total() }}</p>
                        {!! $subscribes->appends(['search' => request('search')])->links() !!}
                    </div>
                </div>
            </div>
        </div>
