    ...
    <div class="breadcrumb-header justify-content-between mb-4">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 ms-2 mb-0">/ Patients Educations</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
                    ...
                    <div class="d-flex justify-content-between">
                        <div class="col-sm-4 col-md-2 col-xl-2">
                            <a class="btn btn-outline-primary btn-block"
                               ...
                               href="{{ route('patients.create') }}">Add Patient Education</a>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table text-md-nowrap" id="table">
                                ...
                                @foreach($patients as $patient)
                                    <tr>
                                        ...
                                        @if(hasEnglish())
                                            <td>{{ $patient->title_en }}</td>
                                        @endif
                                        @if(hasArabic())
                                            <td>{{ $patient->title_ar }}</td>
                                        @endif
                                        <td>{{ $patient->order ?? 0}}</td>
                                        <td>{{ $patient->link }}</td>
                                        <td>
                                            <div
                                                class="toggle-is-feature-btn main-toggle {{ $patient->is_feature == 1 ? 'main-toggle-success on' : 'main-toggle-danger of' }}"
                                                data-id="{{ $patient->id }}"
                                                data-is_feature="{{ $patient->is_feature }}">
                                                <span></span>
                                            </div>
                                        </td>
                                        <td>
                                            <div
                                                class="toggle-status-btn main-toggle {{ $patient->status == 1 ? 'main-toggle-success on' : 'main-toggle-danger of' }}"
                                                data-id="{{ $patient->id }}"
                                        ...
                                        <td>
                                            <a class="btn btn-sm btn-outline-info"
                                               data-bs-effect="effect-scale"
                                               href="{{ route('patients.edit',encrypt($patient->id)) }}"><i
...
@section('scripts')
    <script>
        $(document).on('click', '.toggle-status-btn', function () {
            ...
            $.ajax({
                url: "{{ route('patients.change.status', ['id' => ':id']) }}".replace(':id', patientId),
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
        ...
        $(document).on('click', '.toggle-is-feature-btn', function () {
            ...
            let patientId = button.data('id');
            $.ajax({
                url: "{{ route('patients.change.is_feature', ['id' => ':id']) }}".replace(':id', patientId),
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
