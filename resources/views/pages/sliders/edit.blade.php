    ...
    <div class="breadcrumb-header justify-content-between mb-4">
        <div class="my-auto">
            <div class="d-flex align-items-center">
                ...
                <span class="text-muted mt-1 tx-13 ms-2 mb-0">/ Edit Slider</span>
            </div>
        </div>
    </div>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('sliders.update', $slider->id) }}" method="POST"
                            ...
                            <div class="row">
                                ...
                                @foreach(get_active_langs() as $lang)
                                    <div class="{{ colClass() }} mb-3">
                                        ...
                                        <input type="text" name="title_{{ $lang }}" class="form-control form-control-lg"
                                               value="{{ old('title_'.$lang, $slider->{'title_'.$lang}) }}" required>
                                    </div>
                                @endforeach

                                {{-- Link --}}
                                <div class="col-md-12 mb-3">
                                    <label class="form-label">Destination Link <span
                                            class="text-danger">*</span></label>
                                    <input type="text" name="link" class="form-control"
                                           value="{{ old('link', $slider->link) }}" required>
                                </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="order" class="form-select" required>
                                        @for($i=1 ; $i <= 5 ; $i++)
                                            <option
                                                value="{{ $i }}" {{ $slider->order == $i ? 'selected' : '' }}>{{ $i }}</option>
                                        @endfor
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="step-content" id="step-2">
                            <h5 class="mb-4 text-primary fw-bold border-bottom pb-2">Step 2: Slider Media</h5>
                            <div class="row">
                                @foreach(get_active_langs() as $lang)
                                    <div class="{{ colClass() }} mb-4">
                                        ...
                                        <div class="media-selector-group">
                                            <input type="text" id="banner_{{ $lang }}" name="banner_{{ $lang }}"
                                                   value="{{ $slider->{'banner_'.$lang} ? asset($slider->{'banner_'.$lang}) : '' }}"
                                        ...
                                        <div id="preview_banner_{{ $lang }}">
                                            @if($slider->{'banner_'.$lang})
                                                @php
                                                    $ext = pathinfo($slider->{'banner_'.$lang}, PATHINFO_EXTENSION);
                                                    $isVideo = in_array(strtolower($ext), ['mp4', 'webm', 'mov', 'avi']);
                                                @endphp
                                                <div class="media-preview-card">
                                                    @if($isVideo)
                                                        <video controls width="300" height="180">
                                                            <source src="{{ asset($slider->{'banner_'.$lang}) }}">
                                                        </video>
                                                    @else
                                                        <img src="{{ asset($slider->{'banner_'.$lang}) }}" width="200">
                                                    @endif
                                                </div>
                                            @endif
                                        </div>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="media-selector-group">
                                    <input type="text" id="meta_img" name="meta_img"
                                           value="{{ $slider->meta_img ? asset($slider->meta_img) : '' }}"
                                ...
                                <div id="preview_meta_img">
                                    @if($slider->meta_img)
                                        <div class="media-preview-card">
                                            <img src="{{ asset($slider->meta_img) }}" width="150">
                                        </div>
                                    @endif
                                </div>
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            window.setSingleFile = function (url, type) {
                ...
                let previewHtml = '';
                if (type === 'video') {
                    previewHtml = `<div class="media-preview-card"><video controls width="300" height="180"><source src="${url}"></video></div>`;
                } else {
                    previewHtml = `<div class="media-preview-card"><img src="${url}" width="200"></div>`;
                }
