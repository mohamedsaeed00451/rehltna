    ...
    <div class="breadcrumb-header justify-content-between mb-4">
        <div class="my-auto">
            <div class="d-flex align-items-center">
                ...
                <span class="text-muted mt-1 tx-13 ms-2 mb-0">/ Edit Blog</span>
            </div>
        </div>
    </div>
                            ...
                            <div class="row">
                                ...
                                @foreach(get_active_langs() as $lang)
                                    <div class="{{ colClass() }} mb-3">
                                        ...
                                        <input type="text" name="slug_{{ $lang }}" class="form-control bg-light"
                                               value="{{ old('slug_'.$lang, $blog->{'slug_'.$lang}) }}" readonly>
                                    </div>
                                @endforeach
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select name="category_id" class="form-select" required>
                                        ...
                                        @foreach($categories as $category)
                                            <option
                                                value="{{ $category->id }}" {{ $blog->category_id == $category->id ? 'selected' : '' }}>
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select name="is_feature" class="form-select">
                                        <option value="1" {{ $blog->is_feature == 1 ? 'selected' : '' }}>Feature
                                        </option>
                                        <option value="0" {{ $blog->is_feature == 0 ? 'selected' : '' }}>Not a Feature
                                        </option>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select name="order" class="form-select" required>
                                        @for($i=1 ; $i <= 5 ; $i++)
                                            <option
                                                value="{{ $i }}" {{ $blog->order == $i ? 'selected' : '' }}>{{ $i }}</option>
                                        @endfor
                                    </select>
                                </div>
                            </div>
                                ...
                                @foreach(get_active_langs() as $lang)
                                    <div class="{{ colClass() }} mb-3">
                                        ...
                                        <textarea name="short_description_{{ $lang }}" class="form-control"
                                                  rows="3">{{ old('short_description_'.$lang, $blog->{'short_description_'.$lang}) }}</textarea>
                                    </div>
                                @endforeach
                                ...
                                @foreach(get_active_langs() as $lang)
                                    <div class="{{ colClass() }} mb-4">
                                        ...
                                        <textarea name="description_{{ $lang }}" id="description_{{ $lang }}"
                                                  class="form-control">{{ old('description_'.$lang, $blog->{'description_'.$lang}) }}</textarea>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                                    ...
                                    <div class="{{ colClass() }} mb-3">
                                        ...
                                        <div class="media-selector-group">
                                            <input type="text" id="banner_{{ $lang }}" name="banner_{{ $lang }}"
                                                   value="{{ $blog->{'banner_'.$lang} ? asset($blog->{'banner_'.$lang}) : '' }}"
                                        ...
                                        <div id="preview_banner_{{ $lang }}">
                                            @if($blog->{'banner_'.$lang})
                                                <div class="media-preview-card">
                                                    <img src="{{ asset($blog->{'banner_'.$lang}) }}" width="200">
                                                </div>
                                            @endif
                                        </div>
                                    </div>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="media-selector-group">
                                    <input type="text" id="meta_img" name="meta_img"
                                           value="{{ $blog->meta_img ? asset($blog->meta_img) : '' }}"
                                ...
                                <div id="preview_meta_img">
                                    @if($blog->meta_img)
                                        <div class="media-preview-card">
                                            <img src="{{ asset($blog->meta_img) }}" width="150">
                                        </div>
                                    @endif
                                </div>
                            </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12 mb-3">
                                                ...
                                                <input type="text" name="meta_title_{{ $lang }}" class="form-control"
                                                       value="{{ old('meta_title_'.$lang, $blog->{'meta_title_'.$lang}) }}">
                                            </div>
                                            <div class="col-md-6 mb-3">
                                                <label class="form-label">Meta Description</label>
                                                <textarea name="meta_description_{{ $lang }}" class="form-control"
                                                          rows="3">{{ old('meta_description_'.$lang, $blog->{'meta_description_'.$lang}) }}</textarea>
                                            </div>
                                            <div class="col-md-6 mb-3">
                                                <label class="form-label">Meta Keywords</label>
                                                <textarea name="meta_keywords_{{ $lang }}" class="form-control"
                                                          rows="3">{{ old('meta_keywords_'.$lang, $blog->{'meta_keywords_'.$lang}) }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="d-flex justify-content-between mt-5 pt-3 border-top">
                            ...
                            <div class="ms-auto">
                                ...
                                <button type="submit" class="btn btn-success px-5" id="submitBtn" style="display:none;">
                                    <i class="fas fa-check-circle me-1"></i> Update Blog
                                </button>
                            </div>
                        </div>
