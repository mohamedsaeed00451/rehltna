        ...
        @foreach($testimonials as $testimonial)
            <tr>
                    ...
                    <div class="d-flex align-items-center gap-3">
                        @if($testimonial->image)
                            <img src="{{ asset($testimonial->image) }}" class="client-avatar img-preview"
                                 data-bs-toggle="modal" data-bs-target="#imageModal" data-src="{{ asset($testimonial->image) }}">
                        @else
                        ...
                        <div>
                            ...
                            <span class="text-muted small fw-medium">{{ $testimonial->email }}</span>
                        </div>
                    </div>
                </td>
                <td>
                ...
                <td class="text-center">
                    <div class="toggle-status-btn main-toggle mx-auto {{ $testimonial->status ? 'main-toggle-success on' : 'main-toggle-danger of' }}"
                ...
                <td class="text-center px-4">
                    <div class="d-flex justify-content-center gap-2">
                        <a class="btn-action btn-edit-action" href="{{ route('testimonials.edit', encrypt($testimonial->id)) }}">
                            <i class="las la-pen fs-18"></i>
                        </a>
                        <a class="btn-action btn-delete-action delete-btn" data-route="{{ route('testimonials.destroy', $testimonial->id) }}"
