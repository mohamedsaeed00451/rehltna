    ...
    <div class="breadcrumb-header justify-content-between mb-4">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 ms-2 mb-0">/ Career Types</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
                    ...
                    <div class="d-flex justify-content-between">
                        <div class="col-sm-4 col-md-2 col-xl-2">
                            <a class="btn btn-outline-primary btn-block"
                               ...
                               href="{{ route('career-types.create') }}">Add Type</a>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table text-md-nowrap" id="table">
                                ...
                                @foreach($careerTypes as $careerType)
                                    <tr>
                                        ...
                                        @if(hasEnglish())
                                            <td>{{ $careerType->title_en }}</td>
                                        @endif
                                        @if(hasArabic())
                                            <td>{{ $careerType->title_ar }}</td>
                                        @endif
                                        <td>{{ $careerType->careers_count }}</td>
                                        <td>
                                            <a class="btn btn-sm btn-outline-info"
                                               data-bs-effect="effect-scale"
                                               href="{{ route('career-types.edit',encrypt($careerType->id)) }}"><i
                                                    class="las la-pen"></i></a>
                                            <a class="modal-effect btn btn-sm btn-outline-danger delete-btn"
                                               data-bs-effect="effect-scale"
                                               data-route="{{ route('career-types.destroy',$careerType->id) }}"
