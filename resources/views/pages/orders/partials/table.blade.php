        ...
        @foreach($orders as $order)
            <tr>
                ...
                <td class="text-center">
                    ...
                    @endphp
                    <span class="badge"
                          style="background: {{ $mStyle['bg'] }}; color: {{ $mStyle['color'] }}; border: 1px solid {{ $mStyle['color'] }}40; padding: 8px 14px; border-radius: 10px; font-weight: 800; font-size: 11px;">
                        <i class="fas {{ $mStyle['icon'] }} me-1"></i> {{ strtoupper($order->payment_method) }}
                    </span>
                </td>
                <td class="text-center">
                    @if($order->payment_proof)
                        <a href="javascript:void(0);"
                           class="btn btn-sm btn-outline-success view-proof-btn p-2 rounded-3 border-2"
                ...
                <td class="text-center">
                    ...
                    @endphp
                    <span class="badge"
                          style="background: {{ $sStyle['bg'] }}; color: {{ $sStyle['color'] }}; border: 1px solid {{ $sStyle['color'] }}40; padding: 8px 14px; border-radius: 10px; font-weight: 800; min-width: 90px; text-transform: uppercase; font-size: 11px;">
