...
@section('content')
    ...
    <div class="breadcrumb-header justify-content-between mb-4 mt-3">
        <div class="my-auto">
            ...
            <p class="text-muted mb-0 small fw-medium">Marketing / <span class="text-primary">Inbound Leads</span></p>
        </div>
    </div>

    <div class="hero-section">
        <div>
            <h3 class="mb-2 fw-bold" style="letter-spacing: -0.5px;">Conversion Strategy Hub</h3>
            <p class="mb-0 opacity-75 fw-medium">Manage your lead magnets, track acquisition counts, and toggle
                visibility.</p>
        </div>
        <div class="d-flex gap-3 align-items-center">
            <a href="{{ route('lead-magnets.create') }}"
               ...
               style="background: #ffffff; color: #4f46e5; height: 48px;">
                <i class="fas fa-plus-circle me-2"></i> Add Lead Magnet
            </a>
        </div>
    </div>
            ...
            <div class="card custom-card">
                    ...
                    <div class="table-responsive">
                        <table class="table" id="table">
                            ...
                            <tr>
                                ...
                                @endforeach
                                <th class="text-center">Collected Leads</th>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                            <span class="leads-badge">
                                                <i class="las la-user-plus me-2"></i> {{ $leadMagnet->leads_count }} Leads
                                            </span>
                                    </td>

                                    <td class="text-center">
                                        <div
                                            class="toggle-status-btn main-toggle mx-auto {{ $leadMagnet->status ? 'main-toggle-success on' : 'main-toggle-danger of' }}"
                                            data-id="{{ $leadMagnet->id }}">
                                            <span></span>
                                        </div>
                                    </td>
                ...
                <div class="card-footer bg-transparent border-0 p-4 mt-2">
                    <div class="d-flex justify-content-between align-items-center">
                        <p class="text-muted small mb-0 fw-bold">Records: {{ $leadMagnets->count() }}
                            of {{ $leadMagnets->total() }}</p>
                        {{ $leadMagnets->links() }}
                    </div>
                </div>
            </div>
...
@section('scripts')
    <script>
        ...
        $(document).on('click', '.toggle-status-btn', function () {
            let button = $(this);
            let leadMagnetId = button.data('id');
            $.ajax({
                url: "{{ route('lead-magnets.change.status', ['id' => ':id']) }}".replace(':id', leadMagnetId),
                type: 'POST',
