                            ...
                            <table class="table text-md-nowrap" id="table">
                                ...
                                @foreach($news as $new)
                                    <tr>
                                        ...
                                        <td>
                                            @if($new->type == 'locales')
                                                <span class="text-success">{{ ucfirst($new->type) }}</span>
                                            @else
                                                <span class="text-info">{{ ucfirst($new->type) }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            <div
                                                class="toggle-is-feature-btn main-toggle {{ $new->is_feature == 1 ? 'main-toggle-success on' : 'main-toggle-danger of' }}"
                                                data-id="{{ $new->id }}"
                                                data-is_feature="{{ $new->is_feature }}">
                                                <span></span>
                                            </div>
                                        </td>
                                        <td>
                                            <div
                                                class="toggle-status-btn main-toggle {{ $new->status == 1 ? 'main-toggle-success on' : 'main-toggle-danger of' }}"
                                                data-id="{{ $new->id }}"
                                        ...
                                        <td>
                                            <a class="btn btn-sm btn-outline-info"
                                               data-bs-effect="effect-scale"
                                               href="{{ route('news.edit',encrypt($new->id)) }}"><i
                                                    class="las la-pen"></i></a>
                                            <a class="modal-effect btn btn-sm btn-outline-danger delete-btn"
                                               data-bs-effect="effect-scale"
                                               data-route="{{ route('news.destroy',$new->id) }}"
...
@section('scripts')
    <script>
        $(document).on('click', '.toggle-status-btn', function () {
            ...
            $.ajax({
                url: "{{ route('news.change.status', ['id' => ':id']) }}".replace(':id', newId),
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
        ...
        $(document).on('click', '.toggle-is-feature-btn', function () {
            ...
            let newId = button.data('id');
            $.ajax({
                url: "{{ route('news.change.is_feature', ['id' => ':id']) }}".replace(':id', newId),
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
