    ...
    <div class="breadcrumb-header justify-content-between mb-4">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 ms-2 mb-0">/ Careers</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
                    ...
                    <div class="d-flex justify-content-between">
                        <div class="col-sm-4 col-md-2 col-xl-2">
                            <a class="btn btn-outline-primary btn-block"
                               ...
                               href="{{ route('careers.create') }}">Add Career</a>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table text-md-nowrap" id="table">
                                ...
                                @foreach($careers as $career)
                                    <tr>
                                        ...
                                        <td>
                                            <div
                                                class="toggle-status-btn main-toggle {{ $career->status == 1 ? 'main-toggle-success on' : 'main-toggle-danger of' }}"
                                                data-id="{{ $career->id }}"
                                        ...
                                        <td>
                                            <a class="btn btn-sm btn-outline-info"
                                               data-bs-effect="effect-scale"
                                               href="{{ route('careers.edit',encrypt($career->id)) }}"><i
                                                    class="las la-pen"></i></a>
                                            <a class="modal-effect btn btn-sm btn-outline-danger delete-btn"
                                               data-bs-effect="effect-scale"
                                               data-route="{{ route('careers.destroy',$career->id) }}"
...
@section('scripts')
    <script>
        $(document).on('click', '.toggle-status-btn', function () {
            let button = $(this);
            let careerId = button.data('id');
            $.ajax({
                url: "{{ route('careers.change.status', ['id' => ':id']) }}".replace(':id', careerId),
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
