    ...
    <div class="breadcrumb-header justify-content-between mb-4">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 ms-2 mb-0">/ Edit Event</span>
            </div>
        </div>
    </div>
                        ...
                        <div class="row">
                            ...
                            @foreach(get_active_langs() as $lang)
                                <div class="{{ colClass() }} mb-3">
                                    ...
                                    <input type="text" name="title_{{ $lang }}" class="form-control"
                                           value="{{ old('title_'.$lang, $event->{'title_'.$lang}) }}" required>
                                </div>
                                <div class="{{ colClass() }} mb-3">
                                    <label>Slug ({{ strtoupper($lang) }})</label>
                                    <input type="text" name="slug_{{ $lang }}" class="form-control"
                                           value="{{ old('slug_'.$lang, $event->{'slug_'.$lang}) }}" readonly>
                                </div>
                            @endforeach
                            ...
                            @foreach(get_active_langs() as $lang)
                                <div class="{{ colClass() }} mb-3">
                                    ...
                                    <textarea name="short_description_{{ $lang }}" class="form-control"
                                              rows="3">{{ old('short_description_'.$lang, $event->{'short_description_'.$lang}) }}</textarea>
                                </div>
                            @endforeach
                            ...
                            @foreach(get_active_langs() as $lang)
                                <div class="{{ colClass() }} mb-3">
                                    ...
                                    <textarea name="description_{{ $lang }}" id="description_{{ $lang }}"
                                              class="form-control">{{ old('description_'.$lang, $event->{'description_'.$lang}) }}</textarea>
                                </div>
                            @endforeach
                            ...
                            @foreach(get_active_langs() as $lang)
                                <div class="{{ colClass() }} mb-3">
                                    ...
                                    @if($event->{'banner_'.$lang})
                                        <img src="{{ asset($event->{'banner_'.$lang}) }}" width="100" class="mt-2 rounded">
                                    @endif
                                </div>
                            @endforeach
                                ...
                                @if($event->galleries)
                                    <div class="d-flex flex-wrap gap-2 mt-2">
                                        @foreach($event->galleries as $gallery)
                                            <img src="{{ asset($gallery->image) }}" width="100" class="rounded border p-1">
                                        @endforeach
                                    </div>
                                @endif
                            </div>

                            {{-- Shared Link, Status, Feature, Order --}}
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select name="is_feature" class="form-control">
                                    <option value="1" {{ $event->is_feature == 1 ? 'selected' : '' }}>Feature</option>
                                    <option value="0" {{ $event->is_feature == 0 ? 'selected' : '' }}>Not a Feature</option>
                                </select>
                            </div>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select name="order" class="form-control" required>
                                    ...
                                    @for($i=1 ; $i <= 5 ; $i++)
                                        <option value="{{ $i }}" {{ $event->order == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endfor
                                </select>
                            </div>
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                @if($event->meta_img)
                                    <img src="{{ asset($event->meta_img) }}" width="100" class="mt-2 rounded">
                                @endif
                            </div>
                            ...
                            @foreach(get_active_langs() as $lang)
                                <div class="{{ colClass() }} mb-3">
                                    ...
                                    <input type="text" name="meta_title_{{ $lang }}" class="form-control"
                                           value="{{ old('meta_title_'.$lang, $event->{'meta_title_'.$lang}) }}">
                                </div>
                            @endforeach
                            ...
                            @foreach(get_active_langs() as $lang)
                                <div class="{{ colClass() }} mb-3">
                                    ...
                                    <textarea name="meta_description_{{ $lang }}"
                                              class="form-control">{{ old('meta_description_'.$lang, $event->{'meta_description_'.$lang}) }}</textarea>
                                </div>
                            @endforeach
                            ...
                            @foreach(get_active_langs() as $lang)
                                <div class="{{ colClass() }} mb-3">
                                    ...
                                    <textarea name="meta_keywords_{{ $lang }}"
                                              class="form-control">{{ old('meta_keywords_'.$lang, $event->{'meta_keywords_'.$lang}) }}</textarea>
                                </div>
                            @endforeach
